...
#################################
  ...
  def sysctl_commands():
    pro = subprocess.run(['sudo', 'cp', 'config-files/ubuntu-kernel-config/sysctl.conf', '/etc/sysctl.conf'])
    print(pro.returncode)
...
#######################
  ...
  def ru_commands():
    pro = subprocess.run("sudo apt-get update && sudo apt-get upgrade -y" ,shell=True)
